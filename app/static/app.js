...
async function openPetEditModal(pid){
 const list=await api('/api/pets');
 const p=(list.data||[]).find(x=>Number(x.id)===Number(pid));
 if(!p)return toast('Pet not found','This pet is no longer available.','error');
 const html='<form><div class="modal-grid"><label>Pet name<input name="name" value="'+esc(p.name||'')+'" required></label><label>Species<input name="species" value="'+esc(p.species||'')+'" required></label></div><div class="modal-grid"><label>Breed<input name="breed" value="'+esc(p.breed||'')+'"></label><label>Gender<input name="gender" value="'+esc(p.gender||'')+'"></label></div><div class="modal-grid"><label>Date of birth<input name="date_of_birth" type="date" value="'+esc(p.date_of_birth||'')+'"></label><label>Weight<input name="weight" type="number" step="0.1" value="'+esc(p.weight??'')+'"></label></div><div class="modal-grid"><label>Weight unit<select name="weight_unit"><option value="kg" '+(p.weight_unit==='kg'?'selected':'')+'>kg</option><option value="lb" '+(p.weight_unit==='lb'?'selected':'')+'>lb</option></select></label><label>Color<input name="color" value="'+esc(p.color||'')+'"></label></div><label>Microchip ID<input name="microchip_id" value="'+esc(p.microchip_id||'')+'"></label><label>Allergies<textarea name="allergies" rows="2">'+esc(p.allergies||'')+'</textarea></label><label>Existing conditions<textarea name="conditions" rows="2">'+esc(p.conditions||'')+'</textarea></label><label>Diet<textarea name="diet" rows="2">'+esc(p.diet||'')+'</textarea></label><label>Behavior notes<textarea name="behavior_notes" rows="2">'+esc(p.behavior_notes||'')+'</textarea></label><label>Emergency notes<textarea name="emergency_notes" rows="2">'+esc(p.emergency_notes||'')+'</textarea></label><button class="btn btn-primary" type="submit"><i data-lucide="save"></i>Save changes</button></form>';
 modal('Edit pet details',html,async d=>{await api('/api/pets/'+pid,{method:'PATCH',body:JSON.stringify(d)});toast('Pet updated',p.name+' details have been saved.');setTimeout(()=>location.reload(),450)});
}
async function deletePet(pid,petName){
 const name=petName||'this pet';
 const okDelete=window.confirm('Delete '+name+' and all of its care records? This cannot be undone.');
 if(!okDelete)return;
 try{await api('/api/pets/'+pid,{method:'DELETE'});toast('Pet deleted',name+' and its associated care records were removed.');setTimeout(()=>{location.href='/dashboard#pets'},500)}catch(e){toast('Could not delete pet',e.message,'error')}
}
